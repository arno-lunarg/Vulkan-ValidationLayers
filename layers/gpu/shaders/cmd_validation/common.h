...
void GpuavLogError(uint error_group, uint error_sub_code, uint param_0, uint param_1) {
    ...
    errors_buffer[write_pos + kHeaderErrorGroupOffset] = error_group;
    errors_buffer[write_pos + kHeaderErrorSubCodeOffset] = error_sub_code;

    errors_buffer[write_pos + kPreActionParamOffset_0] = param_0;
    errors_buffer[write_pos + kPreActionParamOffset_1] = param_1;
}
